...
Uses the application settings for the database URL and the ORM ``Base`` metadata
as the autogenerate target, so migrations track the models with no duplication.
"""
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        context.configure(
            ...
            target_metadata=target_metadata,
            render_as_batch=True,  # batch mode = SQLite-safe ALTERs
        )
        with context.begin_transaction():
            context.run_migrations()
