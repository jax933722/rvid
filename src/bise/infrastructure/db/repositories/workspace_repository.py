"""SQLAlchemy implementations of the workspace repositories.

Covers saved searches, company lists (with membership), and company tags. Maps
between ORM models and domain entities so the domain stays persistence-agnostic.
"""
...
class SqlAlchemySavedSearchRepository:
    ...
    def add(self, search: SavedSearch) -> SavedSearch:
        model = SavedSearchModel(name=search.name, query_json=search.query_json)
        self._session.add(model)
        self._session.flush()
        return self._to_entity(model)

    def get(self, search_id: int) -> SavedSearch | None:
        model = self._session.get(SavedSearchModel, search_id)
        return self._to_entity(model) if model is not None else None

    def list(self) -> Sequence[SavedSearch]:
        stmt = select(SavedSearchModel).order_by(
            SavedSearchModel.created_at.desc(), SavedSearchModel.id.desc()
        )
...
class SqlAlchemyCompanyListRepository:
    """Company-list persistence (lists + membership)."""

    def __init__(self, session: Session) -> None:
        self._session = session

    def add(self, company_list: CompanyList) -> CompanyList:
        model = CompanyListModel(name=company_list.name, description=company_list.description)
        self._session.add(model)
        self._session.flush()
        return self._to_entity(model, member_count=0)

    def get(self, list_id: int) -> CompanyList | None:
        model = self._session.get(CompanyListModel, list_id)
        if model is None:
            return None
        return self._to_entity(model, member_count=self._count(list_id))

    def list(self) -> Sequence[CompanyList]:
        stmt = select(CompanyListModel).order_by(
            CompanyListModel.created_at.desc(), CompanyListModel.id.desc()
        )
        return [
...
class SqlAlchemyCompanyTagRepository:
    """Company-tag persistence (normalized labels)."""
    ...
    def add(self, tag: CompanyTag) -> CompanyTag:
        ...
        model = CompanyTagModel(company_id=tag.company_id, label=tag.label)
        self._session.add(model)
        self._session.flush()
        return self._to_entity(model)
    ...
    def list_for_company(self, company_id: int) -> Sequence[CompanyTag]:
        stmt = (
            ...
            .where(CompanyTagModel.company_id == company_id)
